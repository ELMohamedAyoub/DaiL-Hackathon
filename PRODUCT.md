...
One deliberate exception: a downstream, advisory-only "ask about this report" endpoint (`backend/app/routers/c08_ask.py`) answers free-text questions strictly from the already-computed report JSON, must cite an evidence_id for every factual claim, and is instructed to refuse rather than guess when the report doesn't state something. It never re-derives a claim and has no path back into claim construction. Known gap: no programmatic re-validation that the model's citations or numbers actually match the report; currently relies on the system prompt's instructions alone.
