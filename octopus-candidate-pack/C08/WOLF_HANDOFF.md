...
Ask about report: use the collapsible bottom-right chat launcher after triggering a case. It offers suggested example questions when the report is ready and keeps the full multi-turn exchange visible; before a report exists, the input is disabled with "Trigger a case first" guidance.
...
Failure or uncertainty path: completion is rendered as an explicit "not stated, see source assessment record", never inferred from the assessment's content and never a blank. Switch to the `PRG-NOASSESS` demo case to see the same logic when no assessment record exists at all, and `PRG-DOUBLE` to see two separate ambiguous narrative records both correctly excluded by evidence type, not by a hardcoded record ID. `PRG-BULK` is the full 12-week, 17-record case: its eight narrative/non-authoritative evidence types exercise the per-record collapsible accordions for flagged evidence and cross-checks that replace the old flat "show N more" link. Stopping the API and reloading gives a specific recovery instruction rather than a silent failure.
...
| Retrieval / reasoning | Implemented | Evidence is classified by type and narrative records are excluded by type, not ID. Numeric extraction from each authoritative record is three-tier: exactly one digit-run in that record's own text is read deterministically and marked high confidence; zero or multiple digit-runs trigger one DeepSeek call for that genuinely ambiguous record, with the resulting claim explicitly marked low confidence and shown in the UI as "AI-assisted, low confidence"; if that call fails, a fixed known-ID lookup is the last resort and the claim remains low confidence. The LLM use is deliberate and disclosed, limited to the ambiguous minority path, and never used for the clean-text majority. Partner questions and the narrative-vs-register number-mention cross-check remain rules-based (`backend/app/rules/c08_report.py`). |
| Ask about report | Implemented | A collapsible bottom-right chat widget keeps a full multi-turn exchange, offers suggested questions once the report is ready, and disables input with "Trigger a case first" guidance before then. Its downstream DeepSeek endpoint answers only from the already-computed report JSON, must cite an evidence_id per factual claim, and refuses when the report does not state something. Cited IDs are checked against the programme's real evidence IDs and unverifiable answers are withheld; cited numbers are not yet programmatically checked against the record's value (`backend/app/routers/c08_ask.py`). |
